...
import { BracketJoinCell } from '../../components/BracketJoinCell'
...
import { formatKnockoutKickoff } from '../../lib/knockoutSchedule'
...
function PickableTeam({
  ...
  return (
    <button
      type="button"
      className={`scenario-pickable${picked ? ' scenario-picked' : ''}`}
      disabled={disabled}
      aria-pressed={picked}
      onClick={onPick}
    >
      <span className="bracket-flag" aria-hidden>{flagEmoji(team.fifa_code)}</span>
      <span className="scenario-pickable-copy">
...
function KnockoutMatchMeta({ matchNumber }: { matchNumber: number }) {
  const schedule = formatKnockoutKickoff(undefined, matchNumber)
  return (
    <div className="scenario-knockout-meta" aria-label={`Match ${matchNumber} schedule`}>
      <p className="scenario-knockout-when">
        <span>{schedule.day}</span>
        <span>{schedule.date}</span>
        <strong>{schedule.time}</strong>
      </p>
      {schedule.venue && <p className="scenario-knockout-venue">{schedule.venue}</p>}
    </div>
  )
}
...
function KnockoutMatchCard({
  ...
  return (
    <article className={`bracket-match-card scenario-knockout-card${isFinal ? ' final' : ''}${ready ? '' : ' empty'}`}>
      ...
      <KnockoutMatchMeta matchNumber={match.matchNumber} />
    </article>
  )
}
      ...
      <div className="bracket-scroll" ref={scrollRef}>
        <div className="bracket-grid" ref={gridRef}>
          ...
          {JOIN_R32_R16.map((join, index) => (
            <BracketJoinCell key={`j1-${index}`} join={join} column={BRACKET_COL.c1} />
          ))}
          ...
          {JOIN_R16_QF.map((join, index) => (
            <BracketJoinCell key={`j2-${index}`} join={join} column={BRACKET_COL.c2} />
          ))}
          ...
          {JOIN_QF_SF.map((join, index) => (
            <BracketJoinCell key={`j3-${index}`} join={join} column={BRACKET_COL.c3} />
          ))}
          ...
          {JOIN_SF_FINAL.map((join, index) => (
            <BracketJoinCell key={`j4-${index}`} join={join} column={BRACKET_COL.c4} />
          ))}
